...
Cancancan using {appraisals}[https://github.com/thoughtbot/appraisal] to test the code base against multiple versions of rails, as well as the different model adapters.

When first developing, you may need to run +bundle install+ and then +appraisal install+, to install the different sets.

You can then run all appraisal files (like CI does), with +appraisal rake+ or just run a specific set +appraisal rails_3.0 rake+.

See the {spec/README}[https://github.com/bryanrite/cancancan/blob/master/spec/README.rdoc] for more information.
